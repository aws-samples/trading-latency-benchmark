...
import * as TradingBenchmark from '../lib/multi-az-stack';

// Test: VPC is created
test('VPC Created', () => {
    const app = new cdk.App();
    const stack = new TradingBenchmark.TradingBenchmarkMultiAzStack(app, 'MyTestStack');
...
test('Security Group Created and Allows SSH', () => {
    ...
    const resources = template.findResources('AWS::EC2::SecurityGroup', {
        Properties: {
            GroupDescription: 'Allow SSH access and internal traffic for trading benchmark',
            SecurityGroupEgress: [
                {
                    CidrIp: '0.0.0.0/0',
                    IpProtocol: '-1',
