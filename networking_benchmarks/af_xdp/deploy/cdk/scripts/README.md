...
Tradeoff accepted: no last-resort kernel protection against a runaway RT
thread monopolising a core. Mitigated by `isolcpus` (those threads never run
on the CPU0 housekeeping core, so the OS and SSH stay responsive), each
tool's own idle deadline, and the agent wrapping every measurement in
`timeout`.
...
Why the stock config is not enough: chrony's `makestep 1.0 3` only steps during
the first 3 updates and only if the offset exceeds 1s; a smaller post-boot
offset falls through to gradual slewing bounded by `maxslewrate 500` (500ppm),
which can take well over a minute to converge - long enough to run several
benchmark cycles against a skewed clock. `waitsync` blocks (with a timeout)
until chrony reports converged, then `makestep` steps any residual offset.

**Note:** this unit only exists on AMIs baked after it was added - re-bake if
running an older AMI, since a skewed clock without it produces invalid
one-way measurements (hop1 reading 0).
