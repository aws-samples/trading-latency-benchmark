...
func TestApplyRateExpectation(t *testing.T) {
	...
	// (an offered rate exceeding what mcast_send could sustain).
...
// reject the failure mode where a node sits offset because chrony's
// `makestep 1.0 3` declines to step a sub-1s offset.
...
func TestParseRefclockSelected(t *testing.T) {
	...
	if parseRefclockSelected(dualPhcRealFixture) {
		t.Fatal("two PHC lines (PHC0 backup '+', PHC1 selected '*') must still fail " +
			"G5 - exactly one '*' is present, but TWO PHC lines is the duplicate-" +
			"refclock config defect's real signature and must be caught regardless " +
