...
//
// Reads chronyc tracking's "System time", NOT "Last offset". This distinction is
// not cosmetic - it is the difference between catching a broken clock and being
// fooled by one. "Last offset"/"RMS offset" describe how well chrony tracks the
// *rate* of its reference; they say nothing about the absolute error still
// outstanding on the system clock. Measured live on this fleet: a node reported
//
//	System time  : 0.814552128 seconds slow of NTP time
//	Last offset  : -0.000000081 seconds
//	RMS offset   : 0.000000094 seconds
//
// i.e. 814 ms wrong while "Last offset" read 81 ns, because chrony had locked
// the PHC's frequency but was refusing to step a sub-1s offset (makestep 1.0)
// and was slewing it off at maxslewrate 500 - about 27 minutes to converge. Any
// gate keyed on Last offset or RMS offset passes throughout that window and
// admits garbage one-way measurements. See dev/roadmap/precision.md Finding 2.
...
func (r *Runner) ClockSync() (float64, error) {
	...
	// when the clock is hundreds of milliseconds wrong (dev/roadmap/precision.md
	// Finding 2).
	//
...
func (r *Runner) RunMcastReceive(p proto.McastParams) (proto.Metrics, error) {
	...
	// See dev/roadmap/precision.md for the measured clock behaviour.
	if j.Skew > 0 {
...
// ─────────────────────────────────────────────────────────────────────────────
// Wander sampler (dev/roadmap/precision/wander-band-design.md,
// wander-sampler-lifecycle-design.md). StartWander/StopWander bound
...
// (dev/roadmap/precision/phc-nrt/start-samplers.sh) — that pattern exists to
