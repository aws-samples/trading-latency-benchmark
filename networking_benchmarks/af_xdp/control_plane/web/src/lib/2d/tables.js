...
import { relativeAge, ageFade, sparklineSVG } from '../lineage.js';
...
// `variation` and `edgeMap` are optional; when provided, rows gain age annotation.
export function buildPeerTable(fleet, matrix, i, inbound, { variation, edgeMap } = {}) {
  ...
  keys.forEach(pg => {
    ...
    groups[pg].sort((a, b) => a.data.p50 - b.data.p50).forEach(r => { const d = r.data;
      // Age annotation: variation · relative age (3.2).
      const unixSec = d.unix || 0;
      const ageStr = variation ? esc(variation) + ' \u00b7 ' + relativeAge(unixSec, now) : '';
      const fade = unixSec ? ageFade(Math.floor((now - unixSec * 1000) / 1000)) : { opacity: 1, dashed: false };
      const style = fade.opacity < 1 ? ` style="opacity:${fade.opacity}${fade.dashed ? ';border-style:dashed' : ''}"` : '';
      // Look up history for sparkline (if edgeMap provided).
      const edgeKey = edgeMap && (inbound ? fleet.nodes[r.j].private_ip + '|' + fleet.nodes[i].private_ip : fleet.nodes[i].private_ip + '|' + fleet.nodes[r.j].private_ip);
      const edge = edgeKey && edgeMap.get(edgeKey);
      const spark = edge && edge.history ? sparklineSVG(edge.history) : '';
      h += '<tr data-peer="' + r.j + '"' + style + '><td class="peer-name">' + esc(r.peer.public_ip || r.peer.private_ip || r.peer.ec2_name)
        + (ageStr ? '<span class="age-hint"> ' + ageStr + '</span>' : '')
        + '</td><td class="highlight">' + fmtLat(d.p50) + '</td><td>' + (d.p90 ? fmtLat(d.p90) : '\u2014') + '</td><td>' + fmtLat(d.p99) + '</td><td>' + (d.p999 ? fmtLat(d.p999) : '\u2014') + '</td><td>' + (d.max ? fmtLat(d.max) : '\u2014') + '</td><td>' + (d.loss !== undefined ? esc(d.loss) + '%' : '\u2014') + '</td></tr>'
        + (spark ? '<tr class="sparkline-row" data-peer="' + r.j + '"><td colspan="7">' + spark + '</td></tr>' : ''); });
  });
  return h + '</table>';
}
...
// `opts.variation` and `opts.edgeMap` enable age annotation when available.
export function nodeTipHTML(fleet, matrix, i, opts) {
  const node = fleet.nodes[i];
  return '<h3>' + esc(node.public_ip || node.private_ip || node.ec2_name) + roleBadge(node.role) + '</h3>'
    + '<div class="direction">Outbound</div>' + buildPeerTable(fleet, matrix, i, false, opts)
    + '<div class="direction" style="margin-top:6px">Inbound</div>' + buildPeerTable(fleet, matrix, i, true, opts);
}
