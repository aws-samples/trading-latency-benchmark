...
// variations. See App.svelte for the fetch/cache/refresh wiring (auto-refetch
// every 180s while the report is open, plus an explicit Refresh button).
 ...
 *   destination carries. Absent/empty when the caller has no wander data
 *   to offer (e.g. the fetch is still in flight, or WanderSample was off
 *   for every run in this report) - wanderTailTitle then falls back to a
 *   generic caveat text, so the report never shows NO caveat at all on a
 *   mcast tail stat.
 */
function latencyTable(rows, tz, wanderByRun) {
  ...
  // Reporting policy: a one-way (mcast) tail statistic must carry a caveat
  ...
  // fall back to generic "not yet computed" text. RTT (ucast) rows get no
  // title - a single-clock RTT has no inter-node offset to wander.
