...
export function mountControls(host, opts = {}) {
      ...
      <div data-normal>
        ...
        <div class="row">
          <span class="cp-btn-group">
            ...
            <button class="cp-btn" data-run-mcast="all" title="Run the 2 AF_XDP mcast forward modes sequentially (copy → inplace). kernel baseline is opt-in — run it separately.">all</button>
          </span>
        </div>
        </div>
      </div>
  ...
  el.querySelectorAll('[data-run-mcast]').forEach((b) => b.addEventListener('click', () => {
    ...
    const modes = mcastMode === 'all' ? ['copy', 'inplace'] : [mcastMode];
  ...
  el.querySelectorAll('[data-hb-mcast]').forEach((b) => b.addEventListener('click', () => {
    ...
    hbClick(b, { kind: 'mcast', modes: m === 'all' ? ['copy', 'inplace'] : [m] });
  }));
