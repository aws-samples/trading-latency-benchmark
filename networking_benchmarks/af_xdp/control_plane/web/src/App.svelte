<script>
  ...
  // ── Live report overlay ──
  // The report is DB-backed (see module header comment in report-combined.js):
  // it fetches flat rows from the backend's SQLite store instead of reading
  // the live in-memory fleet.matrix, which is keyed only by (kind,variation,
  // src,dst) and can therefore only ever hold ONE value per pair - silently
  // collapsing multi-replicator mcast results to whichever replicator
  // measured most recently. Decoupled from the live SSE stream entirely: the
  // report refetches on open, every REPORT_REFRESH_MS while open, and on an
  // explicit Refresh click - never on a live update tick.
  const REPORT_REFRESH_MS = 180000; // 180s
  let reportRefreshTimer = null;

  async function fetchMeasurements(kind) {
    try {
      ...
      return []; // report still renders (as empty) rather than throwing
    }
  }

  // The live /api/fleet snapshot has one value per (src,dst) edge and gets
  // overwritten as each replicator in a multi-replicator mcast sweep measures
  // the same destinations, so it cannot show more than the LAST replicator's
  // numbers. The store keeps every replicator's run, so the report fetches
  // that history directly - report-only; the live dashboard/3D view still
  // reads from the overwritten snapshot as before.
  // includeMcast: skip this call entirely for a ucast-only report tab - no
  // point fetching mcast replicator history that will be filtered out anyway.
  async function fetchMcastReplicatorResults(includeMcast) {
    if (includeMcast === false) return [];
    try {
      const res = await fetch('/api/mcast-replicators?limit=1000');
      if (!res.ok) return [];
      const body = await res.json();
      return body.results || [];
    } catch {
      return []; // report still renders without the per-replicator section
    }
  }

  // ?report= wants one or more kinds ("ucast", "mcast", or "ucast|mcast").
  // Empty means "no filter" - fetch both kinds, matching the default in-app
  // report state (kind = 'ucast' alone would otherwise wrongly filter it).
  function reportKindFilter() {
    return reportKinds.length === 1 ? reportKinds[0] : '';
  }
  ...
  function openReportOverlay() {
    ...
    if (!reportRefreshTimer) {
      reportRefreshTimer = setInterval(() => { if (reportOverlayOpen) rerenderReportOverlay(); }, REPORT_REFRESH_MS);
    }
  }
  ...
  async function printReport() {
    const measurementRows = await fetchMeasurements(reportKindFilter());
    if (!measurementRows.length) return;
    const mcastReplicatorResults = await fetchMcastReplicatorResults(
      !reportKinds.length || reportKinds.includes('mcast'));
    ...
    const kinds = [...new Set(measurementRows.map((r) => (r.kind === 'mcast' ? 'multicast' : 'unicast')))].join('-');
  ...
  function rerenderReportOverlay() {
    ...
    // The element is bound by Svelte only after the {#if} flushes, and the
    // overlay can open before that. Retry on the next tick rather than
    // failing silently.
    if (!reportOverlayEl) {
      if (!reportRetry) {
    ...
    ensureReportCss();
    // Preserve scroll position and IP selection across re-renders (auto
    // refresh / manual Refresh both land here).
    ...
    Promise.all([
      ...
      finishReportRender(scrollTop, selectedIPs);
    });
  }

  function finishReportRender(scrollTop, selectedIPs) {
    const root = reportOverlayEl.querySelector('.report-content');
    reportInteractions(root);
    // The report's Refresh button dispatches this event rather than calling a
    // closure directly (reportInteractions is serialised via
    // Function.prototype.toString() for the standalone download and must not
    // reference anything outside its own body).
    root.addEventListener('afxdp-report-refresh', () => rerenderReportOverlay());
  ...
  function liveRerender() {
    ...
    updateTargetPanel();
    loading = false; error = '';
    // The report overlay is now DB-backed (see rerenderReportOverlay) and
    // intentionally decoupled from live SSE updates - it does not re-render
    // here. A report tab still renders no map, but has nothing else to do on
    // a live update; remount() is for the 2D/3D dashboard only.
    if (!reportOverlayOpen) remount();
    const s = conn.stats();
  ...
  onMount(async () => {
    ...
    // overlay. The tab opens its own SSE connection (fresh app instance, kept
    // alive for the live dashboard/other tabs; the report itself no longer
    // reads from it - see report-combined.js). No topology is rendered.
    if (params.get('report')) {
      reportKinds = params.get('report').split('|').map((s) => s.trim()).filter(Boolean);
      kind = reportKinds[0] || 'ucast';
      connect();
      openReportOverlay(); // starts the 180s auto-refresh timer, not just the flag
      return;
    }
  ...
  onDestroy(() => { if (handle) handle.dispose(); stopHeartbeat(); if (conn) conn.close(); if (panel) panel.dispose(); if (reportRefreshTimer) clearInterval(reportRefreshTimer); });
</script>
