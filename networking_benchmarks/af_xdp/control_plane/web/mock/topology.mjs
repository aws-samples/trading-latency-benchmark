...
//    Roles: TWO replicators per region (different PG/AZ placement each) so the
//    multi-replicator mcast sweep + report "Per-replicator paths" section has
//    something real to render; sources + destinations as before.
const DEF = [
  ...
  // 2nd replicator: same region, different AZ + PG - the placement comparison
  // the multi-replicator sweep exists to surface.
  ['us-east-1',   '111122223333', 'us-east-1b', 'vpc-use1a', 'cpg-nyc-2', 'cluster', 'c7gn.4xlarge', 'replicator',  1],
  ...
  // 2nd replicator in eu-west-1 too, same shape: different AZ + PG.
  ['eu-west-1',   '444455556666', 'eu-west-1b', 'vpc-euw1b', 'cpg-dub-2', 'cluster', 'c7gn.metal',   'replicator',  1],
...
// Only two ucast variations exist in the real backend today: RunUcastMatrix
// checks `p.Variation == "xdp"` as a single combined flag (both TX and RX
// bypass together) - there is no separate xdp-tx-only / xdp-rx-only mode
// server-side (see orchestrator.go). Matching that here so the mock's Show
// dropdown and report reflect what a real fleet can actually produce, not a
// richer variation set that was never wired into the live orchestrator.
const UCAST_VARS = ['kernel', 'xdp'];
const MCAST_VARS = ['copy', 'inplace'];
// per-variation one-way service latency multiplier vs the kernel baseline
const VAR_MULT = { kernel: 1.0, xdp: 0.61, copy: 0.9, inplace: 0.8 };
...
function buildEdges(nodes) {
  ...
  // Seeded once per (region, mode) using the FIRST replicator's placement as
  // the baseline — this mirrors the real live collector/edgeStore, which is
  // keyed only by (kind,variation,src,dst) and therefore can only ever hold
  // ONE value per destination regardless of how many replicators exist. A
  // live run's bumpEdge() correctly overwrites this per-destination value as
  // whichever replicator most recently measured it — same limitation as the
  // real backend. Multi-replicator comparison lives entirely in
  // /api/mcast-replicators (see recordMcastReplicatorResult in server.mjs),
  // not here; do not try to encode multiple replicators' values into this
  // single-valued edge list.
  MCAST_VARS.forEach((variation, vi) => {
    ['us-east-1', 'eu-west-1'].forEach((region) => {
      ...
      if (!src || !relay) return;
      dests.forEach((d) => {
        // Route the base latency through the (first) replicator, not directly
        // src->dst, so the seed reflects the real two-hop path this fleet's
        // relay placement implies (matches recordMcastReplicatorResult's
        // az-based cost model in server.mjs).
        const hop1 = baseOf.get(pairKey(src.private_ip, relay.private_ip)) || 20;
        const hop2 = baseOf.get(pairKey(relay.private_ip, d.private_ip)) || 20;
        const base = hop1 + hop2;
