...
`N replicators x M modes` wall-clock cost.
...
dispatched `mcast_send`, so loop-elapsed would fold in the sender's start-up
dead time (NATS round trip, exec, ARP/MAC resolution, socket setup) and
understate the achieved rate.
