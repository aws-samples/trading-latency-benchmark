...
// state under which the measured error terms in this project's precision
...
type McastMatrixParams struct {
	...
	// WanderSample toggles the clock-wander sampler for this campaign.
	// Default false: the sampler is opt-in, per-run, and never a persistent
	// background process - it is scoped to exactly one (replicator, mode)
	// run's settle+run window rather than running continuously.
...
// that produced them. A user-facing selector to run a subset of replicators
// instead of always all is a possible future follow-up.
...
func (o *Orchestrator) runMcastForReplicator(p McastMatrixParams, source, replicator registry.Node, dests []registry.Node) {
	...
	for _, mode := range p.Modes {
		...
		// NIC tuning state ("promote from metadata to gate"). Read from
		// source/replicator/first destination - enough to catch a fleet node
		// that drifted from the baked baseline without dispatching to every
		// destination in a large fan-out. Prefix each node's keys with its
		// role so all three show up distinctly in one flat params map instead
		// of colliding.
		...
		// timestamps in a one-way mcast measurement. Leaving it unsynced is a real
		...
		// off at maxslewrate 500 instead (~27 min to converge), producing a bogus
		// hop1=0 / hop2~=665000us split if skipped. CmdClockSync issues an explicit
		// `chronyc makestep`, which ignores the 1.0 s threshold.
				...
				go func() {
					...
					res, err := o.DispatchAgent(d.InstanceID, proto.Command{Type: proto.CmdMcastReceive,
						Mcast: &proto.McastParams{Group: p.Group, DataPort: p.DataPort, Count: p.Count,
							...
							// Omitting it left RequestedPps/RateShortfall
							// permanently unset for every mcast run.
