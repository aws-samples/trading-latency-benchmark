...
// wander figures defined in dev/roadmap/precision/wander-band-design.md §4.
// Pure computation only - no NATS/agent/store dependency, no process
// lifecycle. The orchestrator collects the raw CSV (Stage 2,
// CmdWanderStart/CmdWanderStop) and calls Reduce on it; a later stage
// attaches Fields to the run's stored params and evaluates the §5 gates.
package wander
...
// convention (dev/roadmap/precision/tools/phcsample.c's ioctl-failure path),
// preserved here rather than re-derived from the -1 values so a genuine
// (if implausible) -1 offset can never be silently mistaken for a sentinel.
type Sample struct {
	Seq       int64
	WallNs    int64
	PhcNs     int64
...
// estimator (wander-band-design.md §4's wander_mad_scaled_ns definition).
// This exact constant is what analyze.py already uses (dev/roadmap/precision/
// phc-nrt/analyze.py's mad()) - matching it is what keeps this
// implementation's numbers comparable to every figure already published in
// FINDINGS.md and delay-uncertainty-ledger.md.
...
// the window is too short to see a full correction cycle. Live NRT
// verification (dev/roadmap/precision/live-verification-nrt/, 18 runs
// spanning 1-11s) found raw MAD ranging 168-5732ns, a 34x spread; dividing
// by sqrt(actual sampled duration) - the scaling a diffusive/random-walk
// process's dispersion follows over an observation window - narrowed that to
// 168-2340ns, ~14x. This is a real, substantial, but PARTIAL correction:
...
// sample). The design doc (§4/G3) requires a step to invalidate the whole
// campaign but does not specify a number. 10us is chosen to match
// analyze.py's own EXC tuple (dev/roadmap/precision/phc-nrt/analyze.py:
// EXC = (3_000, 10_000, 50_000)), whose middle tier already represents "a
// materially larger jump than ordinary wander" on this fleet - ordinary
// wander stays within +/-4.7us at p99 per §1, so a 10us consecutive-sample
// jump is well outside anything wander alone produces and is a reasonable,
// documented judgment call rather than an arbitrary one.
...
// nearestRankPercentile matches analyze.py's pctl(): never interpolates, so
// every reported figure is a value that was actually measured
// (dev/roadmap/precision/phc-nrt/analyze.py's own doc comment). q is 0-100.
