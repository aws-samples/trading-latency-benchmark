...
// scale, the exact effect found in live NRT verification
// (dev/roadmap/precision/live-verification-nrt/): two runs with very
// different apparent MAD_scaled, but where most of the difference is
// explained by one run's window being far shorter than the other's -
// duration normalization must narrow the ratio between them, not leave it
// unchanged and not invert it.
...
// TestNearestRankPercentileNeverInterpolates matches analyze.py's own
// contract (dev/roadmap/precision/phc-nrt/analyze.py's pctl() doc comment):
// every returned value must be a value that was actually present in the
// input, never an interpolated point between two samples.
